...
<div class="mt-8">
    <div class="flex justify-between items-center mb-6">
        <h2>Sorotan Utama</h2>
        <a href="{{ route('news.index') }}" class="btn" style="border: 1px solid var(--color-border);">Lihat Semua &rarr;</a>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        @forelse($articles as $article)
            <a href="{{ route('news.show', $article->id) }}" class="card card-hover" style="display: flex; flex-direction: column; color: inherit;">
                <div class="text-xs text-muted mb-2 font-mono uppercase" style="letter-spacing: 0.05em; font-weight: 600;">
                    {{ $article->source }} &middot; {{ $article->published_at ? $article->published_at->diffForHumans() : 'Baru saja' }}
                </div>
                <h3 style="font-size: 1.1rem; margin-bottom: 0.75rem;">{{ $article->title }}</h3>
